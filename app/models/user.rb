class User < ApplicationRecord
  ...
  validates :name, presence: true, format: { with: /\A[a-zA-Z\s]+\z/, message: 'Only letters are allowed' }, length: { in: 3..30 }
  validates :username, presence: true, uniqueness: true, format: { with: /[a-zA-Z0-9]/ }, length: { in: 3..10 }
  ...
  def my_external_sessions
    sessions.includes(:groups).reject { |a| a.groups.any? }
  end
